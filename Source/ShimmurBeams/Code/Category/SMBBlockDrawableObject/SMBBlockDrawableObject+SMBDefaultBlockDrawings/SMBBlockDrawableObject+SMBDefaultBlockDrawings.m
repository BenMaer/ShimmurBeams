...
void SMBBlockDrawableObject_SMBDefaultBlockDrawings__beamCreatorTileEntity_draw(CGContextRef _Nonnull context, CGRect rect, CGColorRef _Nullable powerIndicatorColorRef)
{
	...
	CGContextAddLineToPoint(context,
							CGRectGetMidX(rect) - triangle_inset_from_exit - triangle_width_from_exit + half_line_offset_amount,
							CGRectGetMidY(rect)); /* Left barrel triangle, bottom left. */
	CGContextAddLineToPoint(context,
							CGRectGetMidX(rect) - triangle_inset_from_exit + half_line_offset_amount,
							CGRectGetMidY(rect) - triangle_height_from_exit); /* Left barrel triangle, top right. */
	CGContextAddLineToPoint(context,
							CGRectGetMidX(rect) - triangle_inset_from_exit + half_line_offset_amount,
							CGRectGetMidY(rect)); /* Left barrel triangle, bottom right. */

	CGContextAddLineToPoint(context,
							CGRectGetMidX(rect) + triangle_inset_from_exit + half_line_offset_amount,
							CGRectGetMidY(rect)); /* Right barrel triangle, bottom left. */
	CGContextAddLineToPoint(context,
							CGRectGetMidX(rect) + triangle_inset_from_exit + half_line_offset_amount,
							CGRectGetMidY(rect) - triangle_height_from_exit); /* Right barrel triangle, top right. */
	CGContextAddLineToPoint(context,
							CGRectGetMidX(rect) + triangle_inset_from_exit + triangle_width_from_exit + half_line_offset_amount,
							CGRectGetMidY(rect)); /* Right barrel triangle, bottom right. */
